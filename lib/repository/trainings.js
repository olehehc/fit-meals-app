...
export function getTrainingsByUserAndDateRange(userId, dateFrom, dateTo) {
...
export function getTrainingAndTrainingSetsByUserAndTrainingId(
  trainingId,
  userId
) {
  const training = db
    .prepare("SELECT * FROM trainings WHERE user_id = ? AND id = ?")
...
export function deleteTrainingByUserId(trainingId, userId) {
  ...
  const transaction = db.transaction(() => {
    deletePerformedSets.run(trainingId, userId);

    deleteTrainingSets.run(trainingId, userId);

    const result = deleteTraining.run(trainingId, userId);
